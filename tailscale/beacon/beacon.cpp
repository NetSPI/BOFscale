...
extern "C" BEACON_IMPEX HANDLE BeaconGetStopJobEvent() {
    static HANDLE shutdownEvent = CreateEvent(nullptr, FALSE, FALSE, nullptr);
    return shutdownEvent;
}

extern "C" BEACON_IMPEX int BeaconDownload(const char* filename, const char* buffer, unsigned int length){
    if (!filename || !buffer || length == 0) {
        printf("BeaconDownload: Invalid parameters\n");
        return 0;
    }

    FILE* file = fopen(filename, "wb");
    if (!file) {
        printf("BeaconDownload: Failed to create file '%s'\n", filename);
        return 0;
    }
    ...
    if (written != length) {
        printf("BeaconDownload: Failed to write all data to '%s' (%zu/%u bytes)\n", filename, written, length);
        return 0;
    }

    printf("BeaconDownload: Successfully wrote %u bytes to '%s'\n", length, filename);
    return 1;
}

extern "C" BEACON_IMPEX int BeaconScreenshot(const char* data, int len, const char* filename) {
    if (!data || len <= 0) {
        printf("BeaconScreenshot: Invalid data parameters\n");
        return 0;
    }
    ...
    if (!filename) {
        ...
        snprintf(auto_filename, sizeof(auto_filename),
                 "screenshot_%s_%s_%04d%02d%02d_%02d%02d%02d.bin",
                 hostname, username, st.wYear, st.wMonth, st.wDay,
    ...
    if (!file) {
        printf("BeaconScreenshot: Failed to create file '%s'\n", target_filename);
        return 0;
    }
    ...
    if ((int)written != len) {
        printf("BeaconScreenshot: Failed to write all data to '%s' (%zu/%d bytes)\n", target_filename, written, len);
        return 0;
    }

    printf("BeaconScreenshot: Successfully wrote %d bytes to '%s'\n", len, target_filename);
    return 1;
}

extern "C" BEACON_IMPEX int BeaconKeylog(const char* user, const char* window_title, const char* key_presses) {
    return 1;
}
